...
namespace DatumCollection.MessageQueue.EventBus
{
    /// <summary>
    /// 1. 发布会把消息推送到所有订阅了对应 topic 的消费者
    /// 2. 只能对 topic 做取消订阅，会导致所有订阅都取消。
    /// </summary>
    public class EventBusMQ : IMessageQueue
    {
        private readonly ConcurrentDictionary<string, Action<Message>> _consumers =
            new ConcurrentDictionary<string, Action<Message>>();
        ...
        public EventBusMQ(ILogger<EventBusMQ> logger)
        {
            _logger = logger;
        }
        ...
        private void Publish(string topic, Message message)
        {
            ...
            message.PublishTime = (long)DateTimeHelper.GetCurrentUnixTimeNumber();

            if (_consumers.TryGetValue(topic, out Action<Message> consumer))
            {
                try
                {
                    consumer(message);
